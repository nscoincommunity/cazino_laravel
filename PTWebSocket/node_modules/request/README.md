...
http.createServer(function (req, resp) {
  if (req.url === '/doodle.png') {
    var x = request('https://mysite.com/doodle.png')
    req.pipe(x)
    x.pipe(resp)
  }
})
...
```js
var r = request.defaults({'proxy':'https://localproxy.com'})
...
var r = request.post('https://service.com/upload', function optionalCallback(err, httpResponse, body) {...})
var form = r.form();
...
- `har` - a [HAR 1.2 Request Object](https://www.softwareishard.com/blog/har-12-spec/#request), will be processed from HAR format into options overwriting matching values *(see the [HAR 1.2 section](#support-for-har-1.2) for details)*
