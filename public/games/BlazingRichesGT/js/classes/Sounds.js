...
createjs.Sound.registerSound("/games/BlazingRichesGT/source/SOUND/"+slotSettings.slotSounds[i], sId);	
...
currentSound['FS'].volume=0;	
	...
	if(((cSym=="P_1" && ev.detail.count>=2) || (cSym=="P_2" && ev.detail.count>=3) || (cSym=="P_3" && ev.detail.count>=3) || (cSym=="P_4" && ev.detail.count>=3)) && advancedSound ){
	...
	switch(cSym){
		...
		currentSound['P_4_LONG']=PlaySound("P_4_LONG");	
		delayTime=currentSound['P_4_LONG'].duration;		
...
addEventListener(SLOT_EVENT_STARTSPIN,function(){
	...
	if(soundAdvancedMode==1){

	currentSound['REELSET_START'].volume=0;
...
addEventListener("reelStopSnd",function(ev){
	...
	if(ev.detail.reel==5 && currentSound['REELSET_START']!=undefined){
