...
$(function() {
    $('#language-select').on('click', '.init', function() {
        $(this).closest('ul').children('li:not(.init)').toggle();
    });

    var allOptions = $('#language-select').children('li:not(.init)');
    $('#language-select').on('click', 'li:not(.init)', function() {
        document.cookie = 'language=' + $(this).data('value') + ';path=/';

        allOptions.removeClass('selected');
        $(this).addClass('selected');
        $('#language-select').children('.init').html($(this).html());
        allOptions.toggle();
            ...
            if (!$(this).hasClass('main-nav__item_active')) {
                if (!$(".hero__nav").hasClass("hero__nav_sticky")) {
                    if ($('.main-nav').outerHeight() <= 50) {
                        $('.hero__wrap').animate({
                            height: (subnavHeight + heroHeight) + "px"
                        }, {
                            queue: false,
                            duration: 100
                        });
	...
	$('#showEmailModal').click();
	$('#verifyEmailModal').click()
    ...
    $(document).on('submit', 'form[data-type="ajax"]', function(e) {
        ...
        $.ajax({
			...
			error: function(data){
				...
				if( errors.error ){
					$form.find('.modal__error').show();
					$form.find('.modal__error .modal__note_important,.pay-tooltip__note .error__info').html(errors.error);
				} else {
					$.each(errors, function($key, $value) {
						$form.find('.modal__error').show();
						$form.find('.modal__error .modal__note_important,.pay-tooltip__note .error__info').html('');
						$form.find('.modal__error .modal__note_important,.pay-tooltip__note .error__info').append($value[0] + "<br/>");
					});
				}
			}
        });
	...
	*/
    $(function() {
        $(".activate-bonus").on('click', function(e) {
            ...
            var id = $(this).data('id');
            xhr = $.post('/engine/ajax/activate_bonus.php', {
                'id': id
            }, function(data) {
                xhr = '';
	...
	$(document).on('click', '.countpad', function(e) {
        e.preventDefault();		
        var $answer = $(this).data('answer');		
        $.get($(this).data('target'), function(data) {
            if (data.success) {
				$('#returnsValue').text(data.value);
...
$(function() {
    ...
    $(document).on('click', '[data-toggle]', function(e) {
        ...
        if ($el.attr('data-toggle') == 'add-fav') {
            $.get('/engine/ajax/add_to_favorites.php', {
                'id': $(this).data('id')
            }, function(data) {
                if (data.success) {
            ...
            $.get('/engine/ajax/remove_favorites.php', {
                'id': $el.data('id')
            }, function(data) {
                $el.removeClass('in_favorites');
