...
$steps=array("Installation Wizard - GOLDSVET", "License Agreement", "Check of the installed PHP components", "Checking the write access for important system files", "Data for access to MySQL server", "Installation Complete");

$writable_files=array('../../config/database.php');
...
$dblink = mysqli_connect($dbhost, $dbuser, $dbpassword , $dbname);
	...
	printf("Can\'t connect to the database. Check the username and password", mysqli_connect_error());
	exit();
}

		if(!@mysqli_query($dblink,'SET NAMES utf8'))
			$error = 'Can\'t connect to the database. Check the username and password';
		
		if(!is_readable('install.sql'))
			$error = 'File install.sql not found';
			
		if(!isset($error))
		{
			mysqlrestore('install.sql');
            ...
            $config = str_replace ( "%HOST%" , $dbhost ,$config );
            $config = str_replace ( "%DATABASE%" , $dbname ,$config );
            $config = str_replace ( "%USER%" , $dbuser ,$config );
            $config = str_replace ( "%PASSWORD%" , $dbpassword ,$config );
  ...
  while(!feof($fp)) {
    ...
    if (substr($line, 0, 2) != '--' && $line != '') {
      ...
      if (substr(trim($line), -1, 1) == ';') {
        // Perform the query
        mysqli_query($dblink,$templine) or $error='Error performing query \'<b>' . $templine . '</b>\': ' . mysqli_error() . '<br /><br />';
