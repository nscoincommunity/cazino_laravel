    ...
    <div class="card-body">
        ...
        @else
            @if (settings('registration.captcha.enabled'))
                {!! Form::open(['route' => 'backend.settings.registration.captcha.disable', 'id' => 'captcha-settings-form']) !!}
                <button type="submit" class="btn btn-danger">
                    @lang('app.disable')
                </button>
                {!! Form::close() !!}
            @else
                {!! Form::open(['route' => 'backend.settings.registration.captcha.enable', 'id' => 'captcha-settings-form']) !!}
